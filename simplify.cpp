...
ExpPtr simplify(ExpPtr tree, SimplifyMode mode)
{
    ...
    tree->setLeftNode(simplify(tree->getLeftNode(), mode));
    tree->setRightNode(simplify(tree->getRightNode(), mode));
    ...
    if (tree->getType() == ExpTree::Type::Implies)
    {
        ...
        // A -> @: @
            ...
            return tree->getRightNode();
        // A -> #: !A
        else if (mode != SimplifyMode::Implicational && tree->getRightNode()->isFalsehood())
    ...
    else if (mode != SimplifyMode::Implicational && tree->getType() == ExpTree::Type::Negation)
    {
        // na lógica clássica, !!A: A; na intuicionista, !!!A: !A
        if (tree->getLeftNode()->getType() == ExpTree::Type::Negation &&
            (mode == SimplifyMode::Classical || tree->getLeftNode()->getLeftNode()->getType() == ExpTree::Type::Negation))
    ...
    // Se o nó for do tipo A & B
    else if (mode != SimplifyMode::Implicational && tree->getType() == ExpTree::Type::And)
    {
        // # & A: #
        // A & #: #
    ...
    // Se o nó for do tipo A | B
    else if (mode != SimplifyMode::Implicational && tree->getType() == ExpTree::Type::Or)
    {
        auto node = tree;
        ...
        else if (mode == SimplifyMode::Classical && tree->getLeftNode()->getType() == ExpTree::Type::Negation &&
            *tree->getRightNode() == *tree->getLeftNode()->getLeftNode())
            return ExpTree::True;
        // A | !A: @, não é válido no intuicionista
        else if (mode == SimplifyMode::Classical && tree->getRightNode()->getType() == ExpTree::Type::Negation &&
            *tree->getLeftNode() == *tree->getRightNode()->getLeftNode())
            return ExpTree::True;
        // A | A: A
        else if (*tree->getLeftNode() == *tree->getRightNode())
